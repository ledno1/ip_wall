...
type (
	...
	ISysClientUpdateJ interface {
		// Model 插件客户端更新ORM模型
		Model(ctx context.Context, option ...*handler.Option) *gdb.Model
		// List 获取插件客户端更新列表
		List(ctx context.Context, in *sysin.ClientUpdateJListInp) (list []*sysin.ClientUpdateJListModel, totalCount int, err error)
		// Export 导出插件客户端更新
		Export(ctx context.Context, in *sysin.ClientUpdateJListInp) (err error)
		// Edit 修改/新增插件客户端更新
		Edit(ctx context.Context, in *sysin.ClientUpdateJEditInp) (err error)
		// Delete 删除插件客户端更新
		Delete(ctx context.Context, in *sysin.ClientUpdateJDeleteInp) (err error)
		// View 获取插件客户端更新指定信息
		View(ctx context.Context, in *sysin.ClientUpdateJViewInp) (res *sysin.ClientUpdateJViewModel, err error)
	}
	ISysCron interface {
...
var (
	...
	localSysClientUpdateJ ISysClientUpdateJ
...
func SysClientUpdateJ() ISysClientUpdateJ {
	if localSysClientUpdateJ == nil {
		panic("implement not found for interface ISysClientUpdateJ, forgot register?")
	}
	return localSysClientUpdateJ
}

func RegisterSysClientUpdateJ(i ISysClientUpdateJ) {
	localSysClientUpdateJ = i
}
